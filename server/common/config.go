		...
		form: []Form{
			Form{
				...
				Elmnts: []FormElement{
					...
					FormElement{Name: "host", Type: "text", Default: "https://demo.filestash.app", Description: "The URL that users will use", Placeholder: "Default: https://demo.filestash.app"},
					FormElement{Name: "secret_key", Type: "password", Description: "The key that's used to encrypt and decrypt content"},
					FormElement{Name: "editor", Type: "select", Default: "emacs", Opts: []string{"base", "emacs", "vim"}, Description: "Keybinding to be use in the editor"},
					FormElement{Name: "fork_button", Type: "boolean", Default: true, Description: "Display the fork button in the login screen"},
					FormElement{Name: "display_hidden", Type: "boolean", Default: false, Description: "Visibility or hidden files (files that start with a '.') are shown by default"},
			...
			Form{
				Title: "log",
				Elmnts: []FormElement{
					FormElement{Name: "enable", Type: "enable", Target: []string{"log_level"}},
					FormElement{Name: "level", Type: "select", Default: "INFO", Opts: []string{"DEBUG", "INFO", "WARNING", "ERROR"}, Id: "log_level",  Description: "This setting determines the level of detail at which log events are written to the log file. ERROR: Outputs only error messages. INFO: Outputs error messages and information around startup and initialization. DEBUG: Prints high detail for developers working on debugging issues."},
					FormElement{Name: "telemetry", Type: "boolean", Default: false},
				},
			},
			Form{
				Title: "email",
				Elmnts: []FormElement{
					...
					FormElement{Name: "port", Type: "number", Default: 587, Description: "Port of the SMTP email server", Placeholder: "Default: 587"},
					FormElement{Name: "username", Type: "text", Default: "username@gmail.com", Description: "The username for authenticating to the SMTP server.", Placeholder: "Eg: username@gmail.com"},
					FormElement{Name: "password", Type: "password", Default: "password", Description: "The password associated with the SMTP username.", Placeholder: "Eg: Your google password"},
					FormElement{Name: "from", Type: "text", Default: "username@gmail.com", Description: "Email address visible on sent messages", Placeholder: "Eg: username@gmail.com"},
				},
			},
			Form{
				Title: "auth",
				Elmnts: []FormElement{
					FormElement{Name: "admin", Type: "text", Default: "", Description: "Password of the admin section."},
				},
				Form: []Form{
					Form{
						Title: "custom",
...
func (this Form) MarshalJSON() ([]byte, error) {
	jsonValue := func(el FormElement) string {
		a, e := json.Marshal(el)
		if e != nil {
			return ""
		}
	...
	return []byte(this.toJSON(jsonValue)), nil
}
...
func (this *Configuration) Init() {
	if Config.Get("general.secret_key").String() == "" {
		key := RandomString(16)
		Log.Stdout("SECRET KEY : SET %s", key)
		this.Get("general.secret_key")
		this.Set(key)
	}
	...
	Log.Stdout("SECRET KEY : '%s'", this.Get("general.secret_key").String())
}

func (this Configuration) Save() Configuration {
	Log.Stdout("SAVING %+v", this.form[0].Elmnts[3])
...
func (this Configuration) Export() (string, error) {
	publicConf := struct {
		...
		for i := range *forms {
			...
			if currentForm.Title == path[0] {
				if len(path) == 2 {
					...
					for _, el := range currentForm.Elmnts {
						if el.Name == path[1] {
							return &el
						}
					}
					...
					// we are NOT on a leaf, let's continue our tree trasversal
...
func (this *Configuration) Set(value interface{}) *Configuration {
	...
	if this.currentElement.Value != value {
		...
		Log.Stdout("SET VALUE:\nCURRENT: %+v\nFORM:    %+v", this.currentElement, this.form[0].Elmnts[3])
		this.Save()
	}
	this.mu.Unlock()
	return this
